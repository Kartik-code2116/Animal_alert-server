...
import { Smartphone, Server, Wifi, Copy, CheckCircle, Globe, ArrowRight, Code, BookOpen, Info, AlertTriangle } from 'lucide-react';
...
const STEPS = [
  ...
  },
  {
    ...
    desc: 'Your Android phone needs to reach the server over Wi-Fi. Find your PC\'s IP address.',
  ...
  },
  {
    ...
    desc: 'In the WildTrack app, go to Settings and set the server URL. Use your PC\'s local IP.',
  ...
  },
  {
    ...
    desc: 'Tap the "Test Connection" button in app settings, or open this URL on your phone\'s browser.',
...
const ANDROID_CONFIG = [
  ...
  { label: 'Health timeout', value: '2 seconds', file: 'App.js / server check' },
  { label: 'Auth', value: 'None (planned JWT)', file: 'Phase 1 of upgrade plan' },
  { label: 'Protocol', value: 'HTTP → HTTPS', file: 'Upgrade Phase 1: cert pinning' },
];
  ...
  return (
    <div className="android-guide">
      ...
      <div className="ag-hero">
        ...
        <div className={`pill ${serverStatus === 'online' ? 'pill-online' : 'pill-offline'}`} style={{marginLeft:'auto'}}>
      ...
      <div className="card ag-url-builder">
        ...
        <div className="ag-url-row">
          ...
          <button className="btn btn-primary" onClick={() => copy(serverUrl, 'url')}>
            {copiedIdx === 'url' ? <><CheckCircle size={13}/> Copied!</> : <><Copy size={13}/> Copy URL</>}
          </button>
        </div>
        <div className="ag-url-full mono">{serverUrl}</div>
        <div className="notice-banner info" style={{marginTop:10}}>
          <Info size={13}/>
          Replace the IP with your PC's actual local IP (run <code style={{fontFamily:'var(--font-mono)',background:'rgba(255,255,255,0.06)',padding:'1px 5px',borderRadius:3}}>ipconfig</code> in CMD).
        </div>
      </div>
      ...
      <div className="card" style={{marginTop:4}}>
        <div className="section-header" style={{marginBottom:16}}>
          <h2><Code size={15} style={{display:'inline',marginRight:6}}/>Android App Config Reference</h2>
        </div>
        <div className="table-wrap">
          <table className="wt-table">
            <thead><tr><th>Config Item</th><th>Value</th><th>Location</th></tr></thead>
            <tbody>
      ...
      <div className="card">
        <div className="section-header" style={{marginBottom:16}}>
          <h2><BookOpen size={15} style={{display:'inline',marginRight:6}}/>API Endpoints Used by the App</h2>
        </div>
        <div className="table-wrap">
          <table className="wt-table">
            <thead><tr><th>Method</th><th>Endpoint</th><th>What it does</th><th>Used by</th></tr></thead>
            <tbody>
              {API_ENDPOINTS.map((e,i) => (
                <tr key={i}>
                  <td><span className={`method-badge ${e.method.toLowerCase()}`}>{e.method}</span></td>
                  <td className="primary mono" style={{fontSize:12}}>{e.path}</td>
                  <td style={{fontSize:12}}>{e.desc}</td>
                  <td style={{fontSize:11,color:'var(--text-muted)'}}>{e.used}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="card ag-firewall">
        <div className="section-header" style={{marginBottom:14}}>
          <h2><AlertTriangle size={15} style={{display:'inline',marginRight:6,color:'var(--warn)'}}/>Windows Firewall Setup (if connection fails)</h2>
        </div>
        ...
        <div className="ag-code-block" style={{marginTop:12}}>
          ...
          <button className="ag-copy-btn" onClick={() => copy(`netsh advfirewall firewall add rule name="WildTrack" dir=in action=allow protocol=TCP localport=5000`, 'fw')}>
      ...
      <div className="card ag-upgrade-note">
        <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
          ...
          <div>
            <div style={{fontWeight:700,marginBottom:6,fontSize:14}}>Upgrade Plan Note — Security (Phase 1)</div>
            <div style={{fontSize:12,color:'var(--text-secondary)',lineHeight:1.7}}>
              Currently the connection uses plain HTTP with no authentication.
              Per your <strong style={{color:'var(--text-primary)'}}>Upgrade Plan Phase 1</strong>, this should be replaced with:
              HTTPS + certificate pinning (OkHttp), JWT refresh tokens in EncryptedSharedPreferences,
              and <code style={{fontFamily:'var(--font-mono)',color:'var(--accent)'}}>BuildConfig.BASE_URL</code> instead of a hardcoded IP.
              This dashboard URL builder is a temporary dev tool.
            </div>
          </div>
        </div>
      </div>
